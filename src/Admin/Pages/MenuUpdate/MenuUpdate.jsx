...
const MenuUpdate = ({ menuTitle }) => {
			...
			if (menuData.length === 0) {
				try {
					let res = await axios.post(
						`${import.meta.env.VITE_BACKEND_URL}/api/admin/menu/add`,
						{
							menuKey: menuTitle,
	...
	const setSubMenu = (linkIndex, subIndex, key, value) => {
		...
		updatedLinks[linkIndex].hasSubMenu =
			updatedLinks[linkIndex].subMenu &&
			updatedLinks[linkIndex].subMenu.length > 0;
		setMenuLinks(updatedLinks);
	};
	...
	const deleteSubMenu = (e, linkIndex, subIndex) => {
		e.preventDefault();
		let updatedLinks = [...menuLinks];
		updatedLinks[linkIndex].subMenu = updatedLinks[linkIndex].subMenu.filter(
			(_, idx) => idx !== subIndex
		);
		updatedLinks[linkIndex].hasSubMenu =
			updatedLinks[linkIndex].subMenu.length > 0;
	...
	const submitHandler = async (e) => {
		...
		try {
			let response = await axios.put(
				`${import.meta.env.VITE_BACKEND_URL}/api/admin/menu/edit`,
				submittedMenuData
			);
			if (response.data.status === "Success") menuUpdateNotification();
		} catch (error) {
										...
										<div className="menu-link-item">
											<div>
												...
												<input
													...
													onChange={(e) => {
														setLink(index, "title", e.target.value);
													}}
												/>
											</div>
											<div>
												...
												item.subMenu.map((sub, subIdx) => (
													...
													>
														...
														<input
															...
															onChange={(e) =>
																setSubMenu(index, subIdx, "url", e.target.value)
															}
														/>
														<button
															className="delete-btn"
															onClick={(e) => deleteSubMenu(e, index, subIdx)}
