...
fetchHomepageContent()
  ...
  return (
    <div className="homepage-content">
      { pageData.loading && <div className="loading">Loading...</div> }
      { pageData.error && <div className="error">Error: {pageError}</div> }
      { pageData.data && 
      pageData.data.sections.map((section, index) => {
        (section.data.type ==='banner') ?
          <h1 key={index} className="homepage-title">
            {item.title}
          </h1>
          : <p>Other section type</p>;
      })
      }
      {/* Banner Section  */}
